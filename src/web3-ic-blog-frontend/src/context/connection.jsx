...
const ConnectionProvider = ({ children }) => {
  ...
  const [provider, setProvider] = useState(getReadOnlyProvider(defaultReadonlyChainId));

  const connect = async () => {
    if (window.ethereum === undefined) return toast.info('not an ethereum-enabled browser');
    try {
  ...
  const switchToChain = async (chain) => {
    if (!isSupportedChain(chain)) return toast.error('attempt to switch to a wrong chain!');
    try {
